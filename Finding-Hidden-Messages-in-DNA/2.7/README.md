### $\color{#fffff}{\text{2.7: Implementing RandomizedMotifSearch}}$ 
<details><summary>$\color{#fffff}{\text{Open/Close}}$ </summary> 

<pre>
<b>Python</b>
# ==========================================================
# Project: Finding Hidden Messages in DNA 2.7
# By: Jiyoon Song
# Content: Implementing RandomizedMotifSearch
# ==========================================================
...
def RandomisedMotifSearch(DNA, k, t):
    motifs = []
    for text in DNA:
        start = random.randint(0, len(text) - k) # generating random starting positions
        motifs.append(text[start:start+k]) # add them
    best_motifs = motifs
    while True: # update profile until there's minimum value (until convergence)
        profile = make_profile(motifs)
        new_motifs = [ProfileMostProb(text, profile, k) for text in DNA] # generate new set of motifs based on current profile
        if Score(new_motifs) < Score(best_motifs): # if score is smaller (better)
            best_motifs = new_motifs # update!
            motifs = new_motifs # update!
        else:
            return best_motifs        
        
def RepeatedRandomisedSearch(DNA, k, t, iterations=1000): # repetition (typically 1000)
    best_final_motifs = []
    min_score = math.inf
    for _ in range(iterations):
        current_result = RandomisedMotifSearch(DNA, k, t)
        current_score = Score(current_result)
        if current_score < min_score:
            min_score = current_score
            best_final_motifs = current_result
    return best_final_motifs # from iteration results -> choose best one!
        
print(*RepeatedRandomisedSearch(DNA, k, t, 1000))
